...
export async function runProcessing(formData: FormData) {
...
export async function addProcessingPipeline(formData: FormData) {
...
export async function fetchProcessingPipeline(doc_id: string): Promise<PipelineSpec> {
  const cookieStore = await cookies();
  const token = cookieStore.get("accessToken")?.value;
