...
class Retriever:
    ...
    def __init__(self, db: AsyncSession, logger: InfoLogger, user_id: UUID, level: str):
        self.db = db
    ...
    def _find_source_data(self, filter_ids: Iterable = ()):
        
        # retrieval_ids should correspond to one paragraphs table and level
        retrieval_rows = []
        for filter_id in filter_ids:
            retrieval_rows += self.db.get_all("Retrievals", "retrieval_id=?", filter_id)

        retrieval_df = pd.DataFrame(retrieval_rows)

        input_table = list(set(retrieval_df["source_table"].values))
        filter_level = list(set(retrieval_df["level"].values))
        
        if len(input_table) > 1 or len(filter_level) > 1:
            raise Exception(f"Filter consists of {len(filter_level)} levels in {len(input_table)} tables")
        else:
            filter_level = filter_level[0]
            input_table = input_table[0]



        source_ids = list(set(retrieval_df["source_id"].values))
        source_rows = []
        for id_value in source_ids:
            source_rows += self.db.get_all(input_table, f"{filter_level}_id=?", [id_value])

        source_df = pd.DataFrame(source_rows)
        source_ids = list(set(source_df[f"{self.level}_id"].values))

        return input_table, source_ids


    def _extract_source_table_names(self):

        retrieval_list = self.db.get_all("Retrievals")
        retrieval_df = pd.DataFrame(retrieval_list)

        table_names_list = self.db._query(sql="""SELECT name FROM sqlite_master WHERE type = 'table';""")
        table_names_df = pd.DataFrame(table_names_list)

        source_table_set = set(retrieval_df["source_table"].values)
        mask = (
                table_names_df["name"].str.startswith("paragraphs_") &
                table_names_df["name"].isin(source_table_set)
        )

        table_names = table_names_df.loc[mask, "name"].to_list()

        return table_names


    def _get_retrieval_content(self, retrieval_ids: Iterable = ()):

        retrieval_content = []
        for retrieval_id in retrieval_ids:
            retrieval_content += self.db.get_all("Retrievals", "retrieval_id=?",[retrieval_id])  
        
        retrieval_df = pd.DataFrame(retrieval_content)
    ...
    def _get_retrieval_df(self, filter_ids: Optional[Iterable] = ()):

        # check if there are filters, like document, chapter... from previous retrieval

        if filter_ids:
            source_table, source_ids = self._find_source_data(filter_ids)

            retrieval_content = []
            for source_id in source_ids:
                retrieval_content += self.db.get_all("Retrievals", "source_table=? AND source_id=? AND level=?",
                                                     [source_table, source_id,
                                                      self.level])  # should be a list of only one dict, as these two columns define a one-to-one relationship to the primary key

        else:
            retrieval_content = self.db.get_all("Retrievals", "level=?", [self.level])
...
class ReasonerRetriever(Retriever):
    ...
    def __init__(self, db: AsyncSession, logger: InfoLogger, user_id: UUID, level: str):
        ...
        super().__init__(db, logger, user_id, level)
    ...
    def _get_retrieval_ids(self, query: str, retrieval_input_chunks: str) -> List:
        ...
        system_prompt = """
...

                {
                    ...
                    "properties": {
                        "retrieval_ids": {
                            "title": "retrieval_ids",
                            "description": "A list with each CHUNK_ID that is relevant for this QUERY",
                            "type": "array",
                            "items": {
                                "type": "string"
                            }
        ...
        user_prompt = f"""
            Please analyze following QUERY, CHUNK_IDs, and generate the specified object. 
    ...
    def run_retrieval(self, query: str, filter_ids: Optional[Iterable] = ()):
        start = time.time()

        retrieval_df = self._get_retrieval_df(filter_ids)
...
class EmbeddingRetriever(Retriever):
    ...
    def __init__(self, db: AsyncSession, logger: InfoLogger, user_id: UUID, level: str, embedding_model: str):

        super().__init__(db, logger, user_id, level)  # modern Python 3 style, no super(EmbeddingRetriever, self)
    ...
    def _create_embedding_table(self) -> None:
        ...
        SCHEMA = f"""
                ...
                retrieval_id INTEGER, -- 1-to-1 relationship
                embedding VEC(1536) NOT NULL
                );
                """
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Busy timeout for shorter locks
    ...
    async def generate_embeddings(self, filter_ids: Optional[Iterable] = ()) -> None:
        ...
        async def embed_columns(retrieval_df):
            ...
            embedding_df = pd.DataFrame({"retrieval_id": retrieval_df["retrieval_id"], "embedding": embeddings})
        ...
        rows = await Embedding.get_all({"user_id": self.user_id}, self.db)
        ...
        if not insert_df.empty:
            ...
            self.db.insert_many("Embeddings", dict_list)
        ...
        if not update_df.empty:
            ...
            for embedding_dict in dict_list:
                ...
                self.db.update("Embeddings", embedding_dict, "retrieval_id=?", [embedding_dict["retrieval_id"]])
    ...
    async def run_retrieval(self, query: str, filter_ids: Optional[Iterable] = (), k: int = 3):
        ...
        for _, row in retrieval_df.iterrows():
            ...
            current_item = self.db.get_all(
                "Embeddings",
                "retrieval_id=?",
                [row["retrieval_id"]])
            if not current_item:
                raise TypeError(f"no embeddings found for retrieval_id={row["retrieval_id"]} from table {row["source_table"]}")

            embedding_list += current_item
        ...
        # 5. Sort top-k
        embedding_df = embedding_df.sort_values("cosine_similarity", ascending=False).head(k)

        # extract retrieval_ids from the Retrievals table


        retrieval_output_ids = list(set(embedding_df["retrieval_id"]))
